...
PATH <- "C:\\Users\\sykom\\Desktop\\Stuff\\College Courses\\CS 6302 Predictive Analytics\\Assignments\\Assignment 02\\big.data2_classes.txt"
# path <- file.choose()
```
...
drops <-NULL
for(x in colnames(cell0)[-dim(cell0)[2]]){
   if(max(cell0[x])<2){
      drops <- c(drops, x)
   }
}
...
null <- glm(class ~ 1,data=cell1,family ="binomial")

print("# Remove ANOVA p < 0.01 proteins")
stop1.5 <- dim(cell1.5)[2]  # Need to ignore class variable
...
set.seed(2)
intrain <- createDataPartition(y = cell2$class,p = 0.8,list = FALSE)
assign("training", cell2[intrain,] )
assign("testing",  cell2[-intrain,] )
